...
| `2026-08-31-p25-6/` | Returned P25-6 target evidence from the qualified `a8e974d` package. CPU smoke passes 1/1 planned production cell and CPU screen passes 2/2 planned synthetic cells. Final CUDA evidence is intentionally absent: its first attempt stopped before measurement on the loader collision and then the evaluator's no-CPU-node-fallback defect. |
| `2026-09-01-p25-6/` | Completed P25-6 target run after the CUDA-provider (#29) and failure-publication (#30) fixes. CPU smoke passes 1/1 and CPU screen passes 2/2. Final CUDA now measures all 4 planned cells: the two `live_flame` cells pass, but both `idle` cells fail the 15 GiB hard memory gate at ~21.8 GiB `peak_incremental_device_memory`. Quality metrics are excellent throughout (endpoint error ≈0.001 px). Note the gate flips on baseline subtraction, not candidate behaviour: absolute peak device memory is ~21.6–23.4 GiB of the A5000's 24 GiB in both loads, but `live_flame`'s ~10 GiB resident baseline shrinks the incremental figure below the gate. |
