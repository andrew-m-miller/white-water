...
x right/y down, with a pair displacement from image1 to image2.  The generator is not an
...
`tools/bakeoff/padding.py` implements `replication` (clamp to the edge) and `reflect` (mirror
without repeating the edge pixel, matching `src/core/flow/Preprocess.cpp`).  The current
SEA-RAFT diagnostic caller uses replication while the Phase 2 preprocessing helper uses
reflect.  P25-2 therefore makes the policy an explicit required argument, tests the differing
halo (`[1,2,3]` with one side on each end becomes `[1,1,2,3,3]` versus `[2,1,2,3,2]`), and
does not pretend that either is a universal candidate choice.

P25-1 owns the candidate manifest and its exact padding field.  Once that schema is available,
its adapter should pass the declared string through this narrow seam.  Until then, P25-2 does
not guess a manifest property or modify P25-1's schema.  A cross-candidate comparison is valid
only when the caller-side policy is identical in the comparison cell, exactly as frozen by
`padding_comparison_policy` in `bakeoff/protocol-v1.json`; the asymmetric synthetic case records
replication as its deterministic fixture policy for the current SEA-RAFT contract.
