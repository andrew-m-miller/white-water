...
| **2.5** | Model and export bake-off in `ww-flow` | **In progress. P25-0 through P25-5 are merged (P25-5 through PR #15 at `946c042`, evaluation artifact qualified at `adfd4fb`) and P25-6 packaging is complete (PRs #21–#24):** protocol/schemas, artifact validation, corpus/conditioning, candidate export/evidence, offline runner, active protocol v2, the evaluation-only EL8 airgap package, and the qualified P25-6 target-measurement airgap package (`whitewater-p25-6-el8.tar.gz`, SHA256 `06a72bb5…`; OpenEXR/pynvml runtime, inventory `a87694f5…` approved by the Andrew Miller runtime legal-review) are present. The P25-6 human operator measurement run, ranking, a shipping default, any qualifying fast alternative, persistent choice order, and the measured `analysisScale` decision remain open |
