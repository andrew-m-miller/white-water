...
| **2.5** | Model and export bake-off in `ww-flow` | **In progress. P25-0 through P25-5 are merged (P25-5 through PR #15 at `946c042`, evaluation artifact qualified at `adfd4fb`).** P25-6's runtime, legal inventory and packaging machinery are present, but the first qualified target archive (`whitewater-p25-6-el8.tar.gz`, SHA256 `06a72bb5…`) is withdrawn before measurement: its carried corpus was schema-valid but failed the report-side protocol coverage gate. A corrected package must be reviewed and requalified before the human runs `smoke`, `screen`, or `final`. The P25-6 target run, ranking, a shipping default, any qualifying fast alternative, persistent choice order, and the measured `analysisScale` decision remain open |
