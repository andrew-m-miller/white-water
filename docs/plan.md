...
| **2.5** | Model and export bake-off in `ww-flow` | **In progress. P25-0, P25-1 and P25-2 merged through PRs #6, #8 and #7 respectively:** frozen protocol/schemas, candidate-neutral artifact validation, and deterministic corpus/conditioning are present. P25-3 candidate work is under review in PRs #10-#12; P25-4 runner work is under review in PR #13. Target qualification/measurement, ranking, a shipping default, any qualifying fast alternative, persistent choice order, and the measured `analysisScale` decision remain open |
