...
| **2.5** | Model and export bake-off in `ww-flow` | **In progress. P25-0 through P25-5 are merged (P25-5 through PR #15 at `946c042`, evaluation artifact qualified at `adfd4fb`).** The corrected P25-6 target package was qualified at `a8e974d` (outer SHA256 `b0d2302f…`); its 2026-08-31 CPU smoke and screen reports pass 1/1 and 2/2 cells. CUDA final measurement remains open: the first attempt found that Flame's CUDA directory also shadows the carried ORT unless `ort-cuda12/onnxruntime` comes first, then exposed the evaluator's incompatible no-CPU-node-fallback setting. The replacement keeps CUDA first while permitting ORT's known lower-priority CPU shape nodes. Python ORT rejects a CUDA request that reports CPU first; the native bridge's `selected_providers` field is nominal and does not observe graph placement. Native CUDA session creation proves provider loading only; the final target run needs all five NVML stages as direct execution evidence, with timing treated as performance data rather than proof. CI generated the replacement runtime inventory and Andrew approved its exact `c7b36cc1…` digest; package-carried follow-up changes still require a freshly qualified outer archive before the target rerun. A 2026-09-01 run against `5731b2d` (the publication fix) since completed all four final CUDA cells with accepted NVML: both `live_flame` cells pass, both `idle` cells overrun the 15 GiB resource gate at ~21.8 GiB incremental (published for review in PR #31; see the paragraph below and `docs/context.md` Session 15). Ranking, a shipping default, any qualifying fast alternative, persistent choice order, and the measured `analysisScale` decision remain open |
...
`docs/measurements/2026-09-01-p25-6/`) is the first to complete all four final CUDA cells with
accepted NVML evidence; its `runner.source_commit` is `5731b2d`, main HEAD carrying the publication
fix, so the runner axis no longer records the `a8e974d` mismatch (outer-archive qualification is
still a review-time check). Both `live_flame` cells pass at ~11.4 GiB incremental — the earlier
`BFCArena` failure did not recur — and both `idle` cells fail the 15 GiB gate at 21.837/21.800 GiB
incremental. The gate keys on `peak_device − baseline_device`: absolute peak is ~21.6–23.4 GiB of
the A5000's 24 GiB in **both** loads, so live Flame's ~10 GiB resident baseline, not the candidate,
is what pulls the increment under the gate. Whether P25-7 should gate on absolute peak (headroom to
OOM) is a ranking decision. The P25-6 ranking, shipping choice and target qualification remain
open.
