...
| **NeuFlow v2** | 2024 | Apache-2.0 code; checkpoint-specific terms unresolved | P25-3F provenance and a 432x768 fixed-shape CPU-validated export are hash-bound to upstream commit `204b5e3744461d90303b9ff82caa7a1bb56a2ca2`, but admission is explicitly **excluded pending checkpoint commercial-use and redistribution terms**. The numerical export pass is preserved as evidence; the 432x768 graph is only the first export shape, and other-shape/provider support beyond the recorded CPU run is unverified. No shipping selection or OFX choice index is assigned. |
