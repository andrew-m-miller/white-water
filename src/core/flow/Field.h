...
// `origin` and adjacent nodes are `spacing` full-resolution pixels apart.
//
// This is why there is no upscale pass anywhere in this project. A half-resolution RAFT
// result becomes a field with spacing 2 whose vectors were multiplied by 2 on the way in,
// and from that moment on every consumer -- compose, the ST map, the resampler -- works in
// one coordinate space and never has to know that an analysis scale exists. The
...
struct FieldGeometry {
  ...
  FieldGeometry(Vec2 originValue, double spacingValue)
      : origin(originValue), spacing(spacingValue) {}
  ...
  static FieldGeometry forPixels(int x, int y) {
    return FieldGeometry(Vec2(x + 0.5, y + 0.5), 1.0);
  }
  ...
  static FieldGeometry forPixels(int x, int y, double scale) {
    return FieldGeometry(Vec2(x + 0.5 * scale, y + 0.5 * scale), scale);
  }

  bool operator==(const FieldGeometry &other) const {
    return origin == other.origin && spacing == other.spacing;
  }
...
class Field {
 ...
 public:
  ...
  Vec2 positionOf(int column, int row) const {
    return Vec2(geometry_.origin.x + column * geometry_.spacing,
                geometry_.origin.y + row * geometry_.spacing);
  }
  ...
  void sample(Vec2 position, float *out) const {
    ...
    const double gx = (position.x - geometry_.origin.x) / geometry_.spacing;
    const double gy = (position.y - geometry_.origin.y) / geometry_.spacing;
