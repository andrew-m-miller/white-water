...
# the repo) but would fail on the airgapped box with FileNotFoundError for tools/bakeoff/validator.py.
...
# models/artifact_workflow.py also imports models/exclusion_contract.py at module scope. The NeuFlow
# pack already stages it (as a per-run input); the WAFT pack never did, so it is resolved here too.
exclusion_contract=$(resolve_repo_path "models/exclusion_contract.py")
...
rm -rf "$staging_root"
mkdir -p "$pkg/scripts" "$pkg/models" "$pkg/runtime" "$pkg/legal" "$pkg/legal/source" "$pkg/bakeoff/waft-validation" "$pkg/tools/bakeoff"
...
echo "WAFT-validation: staged-artifact import smoke (exporter closure from the packaged tree)"
( cd "$pkg" && PYTHONDONTWRITEBYTECODE=1 python3.11 - "export_waft" <<'PY'
import importlib
import sys
from pathlib import Path
...
print("WAFT-validation staged-artifact import smoke: PASS")
PY
